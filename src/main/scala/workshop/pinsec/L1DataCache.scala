...
// this ithe second trial of cache based on the vexriscv
...
class DataCache2(val p : DataCacheParameters) extends Component {
  import p._
...
object L1DataCacheMain{
  def main(args: Array[String]): Unit = {

    SpinalVerilog({
      implicit val p = DataCacheParameters(
        cacheSize =4096,
        wayCount = 1,
        refillCount = 2,
        writebackCount = 2,
      ...
      new WrapWithReg.Wrapper(new DataCache2(p)).setDefinitionName("rtl/L1DataCache")
    }).printPruned()
