...
class L2Cache(val p : CacheParam) extends Component {
  import p._
...
//    val way = Reg(UInt(log2Up(cacheWays) bits))
  ...
  val gs = new SlotPool(generalSlotCount)(new GeneralSlot){
    ...
    val fullUpA = slots.map(_.valid).andR
  }
  ...
  val ctrl = new Pipeline{
    ...
    val tags = new Area{
      ...
      val SOURCE_HITS = insert(read.map(t => (t.owners).orR).asBits)
    }
    ...
    val process = new Area{
      ...
      redoUpA.setWhen(preCtrl.FROM_A &&  preCtrl.GS_HIT) //TODO could be less pessimistic
      ...
      val OTHERS = insert(CACHE_LINE.owners) 
      val SELF = insert((CACHE_LINE.owners ).orR)
      ...
      val gsRefill = False
      val gsWrite = preCtrl.WRITE_DATA 
      ...
      redoUpA setWhen( preCtrl.FROM_A )
      ...
      val owners = new Area {
        ...
        val next = (CACHE_LINE.owners.andMask(!clean) ) 
  ...
  val writeBackend = new Pipeline {
    ...
    val inserter = new Area {
      ...
      val beatMax = upABeatsMinusOne

      val LAST = insert(counter === beatMax)
      val addressWord = cmd.address(wordRange)
      val IN_UP_A = insert(counter >= addressWord && counter <= addressWord + upABeatsMinusOne)

      cmd.ready := isReady && LAST
      valid := cmd.valid
      inserterStage(CMD) := cmd.payload
      val addressBase = cmd.address(refillRange) @@ cmd.address(refillRange.low-1 downto 0)
    ...
    val process = new Area {
      ...
      val acMergeLogic = ubp.withDataA generate new Area {
        ...
        val result = (0 until dataBytes).map(i => (selA && BUFFER_A.mask(i))mux(aSplit(i), aSplit(i)))
      }
      val UP_DATA = insert(acMergeLogic.result.asBits)
      val UP_MASK = insert(Bits(ubp.dataBytes bits).setAll())
      if(ubp.withDataA) when(!( CMD.evict)){
      ...
      cache.data.upWrite.arbitrationFrom(toCacheFork.haltWhen( victimHazard))
      ...
      val toDownA = toDownAFork.swapPayload(io.down.a.payloadType)
...
object L2CacheMain extends App{
  ...
  SpinalVerilog(new L2Cache(basicConfig()).setDefinitionName("rtl/L2Cache")).printPruned()
