...
// this s the first trial of the cache based on the spinalhdl example of the DataCache
...
case class AxiDataCacheConfig( cacheSize : Int,
                            bytePerLine : Int,
                            wayCount : Int,
                            addressWidth : Int,
                            cpuDataWidth : Int,
...
// cpu interface
case class DataCacheCpuCmd()(implicit p : AxiDataCacheConfig) extends Bundle{
...
case class DataCacheCpuRsp()(implicit p : AxiDataCacheConfig) extends Bundle{
  val data = Bits(p.cpuDataWidth bit)
}

case class DataCacheCpuBus()(implicit p : AxiDataCacheConfig) extends Bundle with IMasterSlave{
  val cmd = Stream (DataCacheCpuCmd())
  val rsp = Stream (DataCacheCpuRsp())
  ...
  def toAxi4Shared() : Axi4Shared = {
    ...
    // fill cmd bus
    cmd.kind := DataCacheCpuCmdKind.MEMORY  
    cmd.bypass := False
    cmd.all := False
    cmd.valid := mm.sharedCmd.valid
    cmd.wr := mm.sharedCmd.write  
    cmd.address := mm.sharedCmd.addr  
    cmd.data := mm.writeData.data 
    cmd.mask := mm.writeData.strb

    // control ARW bus
    mm.sharedCmd.ready := cmd.ready
    // control W bus
    mm.writeData.ready := cmd.ready
    // control rsp bus 
    rsp.ready := mm.readRsp.ready
    // R bus
    mm.readRsp.valid := rsp.valid 
    mm.readRsp.data := rsp.data
    mm.readRsp.setOKAY()
    //  B bus    
    mm.writeRsp.valid := rsp.valid
    mm.writeRsp.setOKAY()
...
// memory interface
case class DataCacheMemCmd()(implicit p : AxiDataCacheConfig) extends Bundle{
  val wr = Bool()
  val address = UInt(p.addressWidth bit)
...
case class DataCacheMemRsp()(implicit p : AxiDataCacheConfig) extends Bundle{
  val data = Bits(p.memDataWidth bit)
}

case class DataCacheMemBus()(implicit p : AxiDataCacheConfig) extends Bundle with IMasterSlave{
  val cmd = Stream (DataCacheMemCmd())
  val rsp = Stream (DataCacheMemRsp())
  ...
  def toAxi4Shared() : Axi4Shared = {
    ...
    mm.sharedCmd.len := p.burstSize-1 
    ...
    // control R and B bus
    mm.readRsp.ready := rsp.ready
    mm.writeRsp.ready := rsp.ready
...
case class AxiDataCache(implicit p : AxiDataCacheConfig) extends Component{
  import p._
  assert(wayCount == 1)
  ...
  val tagsWriteCmd = Stream(new Bundle{
  ...
  val dataWriteCmd = Stream(new Bundle{
    val way = UInt(log2Up(wayCount) bits)
...
case class Axi4DataCache(implicit p : AxiDataCacheConfig) extends Component{
  import p._
  assert(wayCount == 1)
  assert(cpuDataWidth == memDataWidth)

  val io = new Bundle{
    val axicpu = slave(Axi4Shared(p.getAxi4SharedConfig()))
    val aximem = master(Axi4Shared(p.getAxi4SharedConfig()))
  }

   val cache = new AxiDataCache()(p)

    val cpubus = master(DataCacheCpuBus())
    val membus = slave(DataCacheMemBus())

  io.axicpu <> cpubus.toAxi4Shared()
  io.aximem <> membus.toAxi4Shared()


/*
  val io = new Bundle{
    val axicpu = slave(Axi4(p.getAxi4SharedConfig()))
    val aximem = master(Axi4(p.getAxi4SharedConfig()))
  }
    val axicpu_s = slave(Axi4Shared(p.getAxi4SharedConfig()))
    val aximem_s = master(Axi4Shared(p.getAxi4SharedConfig()))

   axicpu_s <> cpubus.toAxi4Shared()
   aximem_s <> membus.toAxi4Shared()
  io.axicpu <> axicpu_s.toAxi4() 
  io.aximem <> aximem_s.toAxi4()
*/
}


object AxiDataCacheMain{
  def main(args: Array[String]) {

    SpinalVerilog({
      implicit val p = AxiDataCacheConfig(
        cacheSize =4096,
        bytePerLine =32,
        wayCount = 1,
        addressWidth = 32,
        cpuDataWidth = 32,
        memDataWidth = 32)
      new WrapWithReg.Wrapper(new Axi4DataCache()(p)).setDefinitionName("rtl/AxiCache32B")
    }).printPruned()
    ...
    SpinalVerilog({
      implicit val p = AxiDataCacheConfig(
        ...
        cpuDataWidth = 32,
        memDataWidth = 32)
      new AxiDataCache()(p).setDefinitionName("rtl/AxiCache16B")
    }).printPruned()
